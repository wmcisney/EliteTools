...
var Grid = {
    ...
    createTile: function (item) {
        ...
        var showPrintBtn = "printbtnHide";// item.ShowPrintButton ? "printbtnShow" : "printbtnHide";
        ...
        //Grid.Container[0].isContentEditable
        if (Grid.container[0].baseURI.indexOf("Compass") > -1) {
            console.log("this is a compass");
            compassGrid = 1;
        }
        else {
        ...
        if (window.settings.user.isSupervisor) {
            showDelete = 1;
        }
        ...
        if (showDelete == 1) {
            if (compassGrid == 1) {
                var $item = $(
                    ...
                    '<span class="glyphicon glyphicon-trash icon settings trash" onclick="Grid.removeCompassTile(' + id + ')"></span>' +
                    //'<span class="glyphicon glyphicon-edit icon clickable gray" onclick="Grid.saveCompassTile('+ id +')">' +
					...
					function loadIt() {
						...
						if (!scope) {
							...
							if (this.nTimes < 500) {
								...
								showAlert("Error loading tiles. Check your connection.",120000);
							}
							return;
						}
...
removeCompassTile: function (id) {
    $.ajax({
        url: "/Compass/Tile/" + id + "?hidden=True",
        success: function (data) {
            if (showJsonAlert(data, false, true)) {
                $("[data-id='" + id + "']").remove();
                Grid.refreshGrid(false);
