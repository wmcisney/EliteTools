...
$(function () {
	...
	var clickTodoRow = function (evt) {
		...
		$(detailsContents).append(
			...
			"<span class='btn-group pull-right done-button-container'>" +
			"<span class='btn btn-default btn-xs doneButton on-edit-enabled'> Complete</span><input data-todo='" + todo + "' class='todo-checkbox' type='checkbox' " + (checked ? "checked" : "") + "/>" +
			"</span>" +
			...
			"<span class='create-date' style='width:250px;padding-left:21px;color: #999;'>" + clientDateFormat(new Date(createtime)) +
			"</span>" +
			"</span>" +
			"</div>" +
			"</div>");
...
function checkFireworks() {
	...
	if (typeof (seenFireworks) !== "undefined" && !window.fireworksRan && !seenFireworks) {
        if (total > 0) {
            const percentageCompletion = complete / total;
            if (percentageCompletion >= .9) {
                runFireworks(percentageCompletion * 100);
            }
        }
	}
}
...
function runFireworks(percentageDisplay) {
	...
	if (typeof (shootFirework) === "function") {
		shootFirework();
		showModal({
			icon: { icon: "icon fontastic-icon-trophy-1", title: "Congratulations!", color: "#005ED7" },
            title: `${percentageDisplay.toFixed(0)}% To-do completion!`,
			close: function () {
				shootFirework = true;
			}
		});
