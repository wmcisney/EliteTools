...
namespace RadialReview.Controllers {
	public partial class UploadController : BaseController {
		...
		[Access(AccessLevel.UserOrganization)]
		public ActionResult L10(string id = null, long recurrence = 0, bool noheading = false) {
			ViewBag.NoTitleBar = noheading;
			if (string.IsNullOrWhiteSpace(id)) {
				var recurs = L10Accessor.GetVisibleL10RecurrencesVM(GetUser(), GetUser().Id, false);
				return View(recurs);
			}
			...
			dictinary["todos"] = new MvcHtmlString(Template("/upload/sampletodos") + "<h3><b>Instructions:</b> Upload either a .txt file or .csv file.</h3>" +
			...
			dictinary["scorecard"] = new MvcHtmlString(Template("/upload/samplescorecard") + "<h3><b>Instructions:</b> Upload as a .csv file.</h3>" +
			...
			dictinary["issues"] = new MvcHtmlString(Template("/upload/sampleissues") + "<h3><b>Instructions:</b> Upload either a .txt file or .csv file.</h3>" +
			...
			dictinary["rocks"] = new MvcHtmlString(Template("/upload/samplerocks") + "<h3><b>Instructions:</b> Upload either a .txt file or .csv file.</h3>" +
			...
			dictinary["users"] = new MvcHtmlString(Template("/upload/sampleusers") + "<h3><b>Instructions:</b> Upload as a .csv file.</h3>" +
				...
				"");
			dictinary["l10"] = new MvcHtmlString(Template("/upload/samplel10") + "<h3><b>Instructions:</b></h3> <p>1. Download the excel template.</p><p>2. Populate it with your meeting contents.</p><p>3. Re-upload below.</p><br><p><b>Please Note:</b> Users that are marked accountable during uploading will also be added as attendees.</p>");
		...
		private string Template(string url) {
			return @"<a class='pull-right btn btn-info' style='color: white;' href='" + url + @"' target='_blank'>Download template <span class='glyphicon glyphicon-download'></span></a>";
		}
		...
		public async Task<JsonResult> UploadRecurrenceFile(long recurrenceId, HttpPostedFileBase file, UploadType type, bool csv = false) {
			...
			try {
				...
				return Json(ResultObject.CreateError("An error has occurred. File Not Found." + e.Message));
		...
		public async Task<JsonResult> UploadOrgFile(long orgId, HttpPostedFileBase file, UploadType type, bool csv = false) {
			...
			try {
				...
				return Json(ResultObject.CreateError("An error has occurred. The file was not found." + e.Message));
			} catch (FileTypeException e) {
				var err = "File file type cannot be used.";
				if (e.FileType == FileType.XLS || e.FileType == FileType.XLSX) {
					err = "File cannot be in the Excel format (*." + (e.FileType).ToString().ToLower() + ").";
		...
		public async Task<JsonResult> UploadFile(HttpPostedFileBase file) {
			...
			if (file != null && file.ContentLength > 0) {
				// extract only the fielname
				var url = await UploadAccessor.UploadUndocumented(GetUser(), file, "n/upload/", bucket: "tractiontools");
				url = url.Replace("https://s3.amazonaws.com/tractiontools/n/upload/", "https://note.dlptools.com/up/");

				return Json(ResultObject.SilentSuccess(url));
			}
			return Json(new ResultObject(true, ExceptionStrings.SomethingWentWrong));
		}
