...
#pragma warning restore CS0618 // Type or member is obsolete
		...
		public async Task<ActionResult> Advanced() {
			UpdateViewbag();
			OrganizationViewModel model = await GetAdminDataModel(true, false);
			return View(model);
		}
		...
		public async Task<ActionResult> Payment() {
			UpdateViewbag();
			OrganizationViewModel model = await GetAdminDataModel(false, true);
			ViewBag.Credits = PaymentAccessor.GetCredits(GetUser(), GetUser().Organization.Id);

			return View(model);
		}

		private async Task<OrganizationViewModel> GetAdminDataModel(bool testManageOrg, bool testManageFinance) {
			var user = GetUser().Hydrate().Organization().Execute();

			var quarter = await QuarterlyAccessor.GetQuarterOrGenerate(GetUser(), GetUser().Organization.Id);
			...
			var visibleL10s = L10Accessor.GetVisibleL10Meetings_Tiny(GetUser(), GetUser().Id, false, false, teamTypeFilter: Models.L10.L10TeamType.LeadershipTeam);
			var sharedVto = L10Accessor.GetSharedVTOVision(GetUser(), GetUser().Organization.Id);
			var sharedPages = OrganizationAccessor.GetVtoSharedPages(GetUser(), GetUser().Organization.Id);

			var visibleMeetings = visibleL10s.ToSelectList();
			visibleMeetings.Insert(0, new SelectListItem() {
				Text = "<No meeting>",
				Value = "0"
			});
			...
			var model = new OrganizationViewModel() {
				...
				EmployeesCanEditSelf = true,// user.Organization.Settings.EmployeesCanEditSelf,
				...
				AllVisibleMeetings = visibleMeetings,
				SelectedShareVTORecurrenceId = sharedVto ?? 0,

				CurrentQuarter = new QuarterVM(quarter),

				ShareVtoPages = sharedPages,
		...
		[HttpPost]
		[Access(AccessLevel.Manager)]
		public async Task<ActionResult> Advanced(OrganizationViewModel model) {
			await OrganizationAccessor.Edit(
				GetUser(),
				model.Id,
				...
				model.SelectedShareVTORecurrenceId,
				model.ShareVtoPages
				);
			...
			return Redirect(nameof(Advanced));
		}
	}
}
