...
namespace RadialReview.Accessors {
	...
	public class UploadAccessor {
		public static async Task<UploadModel> UploadFile(UserOrganizationModel caller, UploadType type, HttpPostedFileBase file, ForModel forModel = null) {
			return await UploadFile(caller, type, file.ContentType, file.FileName, file.InputStream, forModel);
		}

		public static async Task<UploadModel> UploadFile(UserOrganizationModel caller, UploadType type, String contentType, String originalName, Stream stream, ForModel forModel = null) {
		...
		public static async Task<UploadInfo> UploadAndParseExcel(UserOrganizationModel caller, UploadType type, HttpPostedFileBase file, ForModel forModel) {
		...
		public static async Task<UploadInfo> UploadAndParse(UserOrganizationModel caller, UploadType type, HttpPostedFileBase file, ForModel forModel) {
			if (file != null && file.ContentLength > 0) {
				using (var ms = new MemoryStream()) {
					await file.InputStream.CopyToAsync(ms);
		...
		public static async Task<UploadInfo> DownloadAndParse(UserOrganizationModel caller, string path) {
			...
			using (var ms = new MemoryStream()) {
				if (path.StartsWith("noaws_")) {
					...
					using (var webClient = new WebClient()) {
						var data = await webClient.DownloadStringTaskAsync("https://elitetools.s3.us-east-2.amazonaws.com/" + path);
		...
		public static async Task<string> UploadUndocumented(UserOrganizationModel caller, HttpPostedFileBase file, string folder, string bucket = "Radial") {
			...
			using (var s = HibernateSession.GetCurrentSession()) {
				using (var tx = s.BeginTransaction()) {
					...
					return Path.Combine(ConstantStrings.AmazonS3BaseLocation, bucket) + "/" + path;
					//return Path.Combine(ConstantStrings.AmazonS3BaseLocation,bucket,path);
				}
			}
		}
