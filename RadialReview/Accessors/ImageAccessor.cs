...
namespace RadialReview.Accessors {
	public class ImageAccessor : BaseAccessor {
		...
		public static async Task Upload(Stream stream, string path, Instructions instructions) {
			using (var ms = new MemoryStream()) {
				...
				var fileTransferUtility = new TransferUtility(new AmazonS3Client(Amazon.RegionEndpoint.USEast2));
		...
		private static async Task<ImageModel> RawUploadImage(UserModel uploader, string filename, Stream inputStream, UploadType type, bool huge = false) {
			...
			using (var s = HibernateSession.GetCurrentSession()) {
				using (var tx = s.BeginTransaction()) {
					...
					await Upload(sBig, path, BIG_INSTRUCTIONS);
					await Upload(sTiny, pathTiny, TINY_INSTRUCTIONS);
					await Upload(sMed, pathMed, MED_INSTRUCTIONS);
					await Upload(sLarge, pathLarge, LARGE_INSTRUCTIONS);

					if (huge) {
						await Upload(sHuge, pathHuge, HUGE_INSTRUCTIONS);
					}
