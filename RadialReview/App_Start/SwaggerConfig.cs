		...
		public static void Register() {
			...
			GlobalConfiguration.Configuration
				.EnableSwagger(c => {
					...
					//
					//c.SingleApiVersion("v1", "Elite Tools API").Contact(x=>x.Url("https://traction.tools"));
					/*	.Description(@"
<h1 id=""Overview"">Overview</h1>
	...
	<div class=""doc-section"">
		...
		<p></p>
		<p>The post request is delivered to <b>https://traction.tools/Token</b>. The following is a curl.</p>
		<figure class=""highlight"">
			<code>curl -i -X POST -d ""grant_type=password"" -d ""userName=<b>YOUR_USERNAME_HERE</b>"" -d ""password=<b>YOUR_PASSWORD_HERE</b>"" 'https://traction.tools/Token'</code>
		</figure>
...
"".expires"":""Mon, 11 Jan 2016 07:05:37 GMT""
		...
		<div class=""panel panel-default"">
			...
			<div class=""panel-body"">
				<code>curl -i -X GET -H ""Authorization:Bearer <b>YOUR_BEARER_TOKEN_HERE</b>"" 'https://traction.tools/api/v0/scorecard/items/'</code>
			</div>
		</div>
	</div>
						...
						");*/
					//.License(x => x.Name("L").Url("lurl")).Contact(x => x.Name("nam").Email("email").Url("url")).TermsOfService("tos");
					...
					c.MultipleApiVersions(
					    ...
					    (vc) =>{
					        vc.Version("v1", "Elite Tools API - V1");
					        vc.Version("v0", "Elite Tools API - V0");
					    });
                    ...
                    //c.s.Customize(cc =>
                    //{
                    //	cc.GroupDeclarationsBy(apiDesc => apiDesc.RelativePath.Replace("/", "|"));
                    //}
				...
				.EnableSwaggerUi(c => {
					...
					c.DocumentTitle("Elite Tools API");
					...
					//====== HEY!! ======
					//Embed the resource with the properties menu. Copy if newer. Find the file in the bin folder
					//Logical name is: "RadialReview.{Path_to_file"}
					c.InjectStylesheet(typeof(ApplicationWideModel).Assembly, "RadialReview.Content.Swagger.CustomSwaggerStyle.css");
					c.InjectJavaScript(typeof(ApplicationWideModel).Assembly, "RadialReview.Scripts.Swagger.CustomSwaggerScripts.js");
		...
		private static bool ResolveVersionSupportByRouteConstraint(ApiDescription apiDesc, string targetApiVersion) {
			return apiDesc.RelativePath.ToLower().Contains("/" + targetApiVersion.ToLower() + "/");
		}
    ...
    //[AttributeUsage(AttributeTargets.Method|AttributeTargets., AllowMultiple = true)]
    ...
    public class AddOptionalHeaderParameter : IOperationFilter
    {
        public void Apply(Operation operation, SchemaRegistry schemaRegistry, ApiDescription apiDescription)
        {
            ...
            var hasCustomerAttribute = apiDescription.GetControllerAndActionAttributes<TTActionWebApiFilter>().Any();
            if (hasCustomerAttribute)
            {
                operation.parameters.Add(new Parameter
                {
                    ...
                    @in = "header",
                    type = "boolean",
                    description = "Include permission in the response",
                    required = false
                });                
            }            
        }
