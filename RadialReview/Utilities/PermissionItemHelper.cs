    ...
    public partial class PermissionsUtility {
		...
		public IEnumerable<long> GetIdsForResourceThatUserIsMemberOf(PermItem.ResourceType resourceType, long userId, bool ignoreExceptions = false) {
			try {
				switch (resourceType) {
					...
					case PermItem.ResourceType.Compass:
						var orgId = session.Get<UserOrganizationModel>(userId).Organization.Id;
						return session.QueryOver<Compass>()
							.Where(x => x.DeleteTime == null && x.ForOrganization == orgId && x.ForMeeting > 0)
							.Select(x => x.ForMeeting)
							.Future<long>().Distinct();
					case PermItem.ResourceType.AccountabilityHierarchy:
		...
		protected bool IsCreator(PermItem.ResourceType resourceType, long resourceId) {
			switch (resourceType) {
				...
				case PermItem.ResourceType.EditDeleteUserDataForOrganization:
					return false;
				case PermItem.ResourceType.File:
					return (session.Get<EncryptedFileModel>(resourceId).CreatorId == caller.Id);
				default:
					throw new ArgumentOutOfRangeException("resourceType");
			}
		}
		...
		protected long GetOrganizationId(PermItem.ResourceType resourceType, long resourceId) {
			switch (resourceType) {
				...
				case PermItem.ResourceType.EditDeleteUserDataForOrganization:
					return resourceId;
				//case PermItem.ResourceType.File: /*do we want this?*/
				//	return (session.Get<EncryptedFileModel>(resourceId).OrgId);
				default:
					throw new ArgumentOutOfRangeException("resourceType");
			}
		}
